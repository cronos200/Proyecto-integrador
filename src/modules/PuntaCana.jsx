...
const PuntaCana = () => {
  ...
  useEffect(() => {
    fetch("/fake-api.json")
      .then((res) => res.json())
      .then((data) => {
        const puntaCanaPlan = data.planesTuristicos.find(
          (p) => p.destino === "Punta Cana"
        );
        setPlan(puntaCanaPlan);
        setNoches(puntaCanaPlan.nochesBase);
      });
  }, []);
  ...
  return (
    <div className="destinos-container">
      <div className="destinos-image">
        <img src="/images/PuntaCanaDos.jpg" alt="Punta Cana" />
      </div>

      <div className="destinos-content">
        <h2>Punta Cana</h2>
        <p>
          Punta Cana es un hermoso destino tropical en la costa este de la
          República Dominicana, conocido por sus playas de arena blanca, aguas
          cristalinas y una atmósfera relajada que combina naturaleza
          exuberante, historia caribeña y una animada vida turística.
        </p>
